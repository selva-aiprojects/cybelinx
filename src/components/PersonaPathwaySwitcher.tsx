...
export default function PersonaPathwaySwitcher({ className = "" }: { className?: string }) {
  ...
  return (
    <div className={`inline-flex max-w-full flex-wrap items-center rounded-2xl border border-border/80 bg-white/80 dark:bg-slate-900/80 p-1.5 backdrop-blur-xl shadow-lg ${className}`} role="group" aria-label="Viewing pathway">
      ...
      <div className="flex flex-wrap items-center gap-1">
        ...
        >
          ...
          <span>Executive<span className="hidden sm:inline"> (Business ROI)</span></span>
        </button>
        ...
        >
          ...
          <span>Engineering<span className="hidden sm:inline"> (Deep Tech)</span></span>
        </button>
      </div>
    </div>
  );
